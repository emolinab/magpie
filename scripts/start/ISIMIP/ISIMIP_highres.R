...
resolution<-c("c500","c1000","c2000","c3000")#,"c800","c1000")

for(re in resolution){
  for (ru in 1:length(runs)){

  dir.create("output/",re)
  cfg$results_folder <- paste0("output/",re,"_041121_ndc/:title:")
  ...
  cc<-if(grepl(c, runs[ru], fixed=TRUE)) c else cc
     ...
     cc_re[ru]<-cc
      ...
      cfg$title <- paste("ISIMIP_041121_ndc",rcp_re[ru],gcm_re[ru],cc_re[ru],re,sep="_")
      ...
      gdx<-paste0("output/c200_041121_ndc/",as.character(subset(c200_Runs,rcp==rcp_re[ru] & gcm==gcm_re[ru] & scenario==cc_re[ru])[1,"name"]),"/fulldata.gdx")
