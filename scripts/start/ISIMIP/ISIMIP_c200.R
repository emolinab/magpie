...
dir.create("output/c200_120122")
cfg$results_folder <- "output/c200_120122/:title:"
  ...
  for(g in 1:length(gcms)){
    ...
    for(c in 1:length(climate)){
      ...
      cfg$input <- c(cellular    = as.character(subset(cell_input,rcp==scenarios[s] & gcm==gcms[g] & resolution == "c200")[1,"name_tgz"]),
                     ...
                     calibration = "calibration_H13_sticky_feb18_dynamic_12Jan22.tgz")
      ...
      cfg$title <- paste("ISIMIP_110122_ndc",scenarios[s],gcms[g],climate[c],sep="_")
